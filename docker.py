...
class Dclient(object):
	...
	def create_and_start_container(self,image,name,expose=None,host_port=None,container_port=None,host_path=None,
		container_path=None,mode=None,env_file=None
		):
		'''create and start container
		   The expected format of the env_file is as follows:
			USERNAME=jdoe
			PASSWORD=secret
		'''
		...
		if env_file != None:
			envs = docker.utils.parse_env_file(env_file)

		con = self.cli.create_container(image=param['image'],name=param['name'],
					ports=ports,host_config=host,environment=envs
				)
...
if __name__ == "__main__":
	...
	stat = d.create_and_start_container(
				name = "mydb",
				image = "mysql",
				expose = "3306",
				host_path = "/data",
